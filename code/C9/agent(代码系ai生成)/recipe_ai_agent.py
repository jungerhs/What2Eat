...
# -*- coding: utf-8 -*-
"""
基于Kimi API的智能菜谱解析AI Agent - 支持增量更新
"""
...
class RecipeKnowledgeGraphBuilder:
    ...
    def _load_manifest(self) -> Dict[str, FileManifestEntry]:
        """加载文件清单"""
        ...
        if os.path.exists(manifest_file):
            try:
                ...
                for file_path, entry in raw.items():
                    manifest[file_path] = FileManifestEntry(**entry)
            except Exception as e:
                print(f"警告: 加载清单文件失败 - {str(e)}")
        return manifest

    def _save_manifest(self, manifest: Dict[str, FileManifestEntry]):
        """保存文件清单"""
        manifest_file = os.path.join(self.output_dir, "manifest.json")
        raw = {}
        for file_path, entry in manifest.items():
            raw[file_path] = {
                ...
                "recipe_concept_id": entry.recipe_concept_id,
    ...
    def _remove_concept_and_relationships(self, concept_id: str, manifest_entry: FileManifestEntry = None):
        """从当前批次中移除指定概念及其相关关系"""
        self.concepts = [c for c in self.concepts if c["concept_id"] != concept_id]
        self.relationships = [r for r in self.relationships
                              if r["source_id"] != concept_id and r["target_id"] != concept_id]
        if manifest_entry:
            for ing_id in manifest_entry.ingredient_ids:
                self.concepts = [c for c in self.concepts if c["concept_id"] != ing_id]
            for step_id in manifest_entry.step_ids:
                self.concepts = [c for c in self.concepts if c["concept_id"] != step_id]

    def _classify_changed_files(self, recipe_dir: str, manifest: Dict[str, FileManifestEntry]) -> Tuple[List[str], List[str], List[str]]:
        """将文件分类为: 新增、修改、删除"""
        import glob
        ...
        current_paths = set()
        for recipe_file in filtered_files:
            relative_path = os.path.relpath(recipe_file, recipe_dir)
        ...
        for recipe_file in filtered_files:
            ...
            if relative_path not in manifest:
                new_files.append(recipe_file)
            else:
                entry = manifest[relative_path]
                current_mtime = os.path.getmtime(recipe_file)
                if current_mtime != entry.mtime:
                    current_hash = self._compute_file_hash(recipe_file)
                    if current_hash != entry.file_hash:
                        modified_files.append(recipe_file)

        deleted_paths = manifest_paths - current_paths

        return new_files, modified_files, list(deleted_paths)

    def incremental_process_recipes(self, recipe_dir: str) -> Tuple[int, int, int, int]:
        """增量处理菜谱目录 - 只处理新增和修改的文件

        Returns:
            Tuple[新增数量, 修改数量, 删除数量, 失败数量]
        """
        manifest = self._load_manifest()
        new_files, modified_files, deleted_paths = self._classify_changed_files(recipe_dir, manifest)

        print(f"📊 增量扫描结果:")
        print(f"   - 新增文件: {len(new_files)}")
        print(f"   - 修改文件: {len(modified_files)}")
        print(f"   - 删除文件: {len(deleted_paths)}")

        if not new_files and not modified_files and not deleted_paths:
            print("✅ 没有变更，知识图谱已是最新!")
            return 0, 0, 0, 0
        ...
        for recipe_file in new_files:
            try:
                with open(recipe_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                relative_path = os.path.relpath(recipe_file, recipe_dir)
                recipe_concept = self.process_recipe(content, relative_path)

                entry = FileManifestEntry(
                    file_path=relative_path,
                    mtime=os.path.getmtime(recipe_file),
                    file_hash=self._compute_file_hash(recipe_file),
                    recipe_concept_id=recipe_concept["concept_id"],
                    ingredient_ids=self._last_ingredient_ids,
                    step_ids=self._last_step_ids,
                    relationship_ids=self._last_relationship_ids
                )
                ...
                if incremental_batch_count >= self.batch_size:
                ...
                failed_count += 1
                print(f"❌ 处理新增文件失败: {recipe_file} - {str(e)}")

        for recipe_file in modified_files:
            try:
                with open(recipe_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                relative_path = os.path.relpath(recipe_file, recipe_dir)
                entry = manifest.get(relative_path)
                old_concept_id = entry.recipe_concept_id if entry else None

                recipe_concept = self.process_recipe(content, relative_path, existing_recipe_id=old_concept_id)

                if old_concept_id and old_concept_id != recipe_concept["concept_id"]:
                    self._remove_concept_and_relationships(old_concept_id, entry)
                ...
                manifest[relative_path] = new_entry
                self._save_manifest(manifest)
                modified_count += 1
                incremental_batch_count += 1
                ...
                failed_count += 1
                print(f"❌ 处理修改文件失败: {recipe_file} - {str(e)}")
        ...
        for deleted_path in deleted_paths:
            try:
                entry = manifest.get(deleted_path)
                if entry and entry.recipe_concept_id:
                    self._remove_concept_and_relationships(entry.recipe_concept_id, entry)
                del manifest[deleted_path]
                self._save_manifest(manifest)
                deleted_count += 1
            except Exception as e:
                print(f"❌ 移除已删除文件失败: {deleted_path} - {str(e)}")

        return new_count, modified_count, deleted_count, failed_count
    ...
    def merge_all_batches(self):
        """合并所有批次数据到最终输出文件（自动过滤增量更新中的过时数据）"""
        print("合并批次数据...")

        manifest = self._load_manifest()
        active_concept_ids = set()
        for entry in manifest.values():
            active_concept_ids.add(entry.recipe_concept_id)
            active_concept_ids.update(entry.ingredient_ids)
            active_concept_ids.update(entry.step_ids)
        ...
        for batch_dir in batch_dirs:
            ...
            if os.path.exists(concepts_file):
                ...
                if active_concept_ids:
                    batch_concepts = batch_concepts[batch_concepts["concept_id"].isin(active_concept_ids)]
            ...
            if os.path.exists(relationships_file):
                ...
                if active_concept_ids:
                    batch_relationships = batch_relationships[
                        batch_relationships["source_id"].isin(active_concept_ids) |
                        batch_relationships["target_id"].isin(active_concept_ids)
                    ]
                all_relationships.append(batch_relationships)

        if all_concepts:
            final_concepts = pd.concat(all_concepts, ignore_index=True).drop_duplicates(subset=["concept_id"])
            final_concepts.to_csv(os.path.join(self.output_dir, "concepts.csv"),
                                index=False, encoding='utf-8')
            print(f"合并概念: {len(final_concepts)} 个")

        if all_relationships:
            final_relationships = pd.concat(all_relationships, ignore_index=True).drop_duplicates(subset=["relationship_id"])
    ...
    def process_recipe(self, markdown_content: str, file_path: str, existing_recipe_id: str = None) -> Dict:
        ...
        recipe_id = existing_recipe_id or self.generate_concept_id()
        ...
        for ingredient in recipe_info.ingredients:
            ...
            rel_id = f"R_{len(self.relationships) + 1:06d}"
            ...
            self._last_relationship_ids.append(rel_id)
