...
def main():
    ...
    try:
        # 检查是否为增量模式
        is_incremental = len(sys.argv) > 2 and sys.argv[2] == "--incremental"
        ...
        if is_incremental:
            ...
            print(f"增量处理结果: 新增 {new_count} 个，修改 {modified_count} 个，删除 {deleted_count} 个，失败 {failed} 个")
        else:
            # 批量处理菜谱
            print(f"\n📚 开始处理菜谱目录...")
...
基本用法:
  python run_ai_agent.py [菜谱目录路径]           # 全量处理
  python run_ai_agent.py [菜谱目录路径] --incremental  # 增量处理（仅处理变更文件）
...
示例:
  ...
  python run_ai_agent.py ./HowToCook-master --incremental
  python run_ai_agent.py test
