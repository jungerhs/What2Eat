...
def show_manifest(output_dir: str):
    ...
    try:
        ...
        print(f"📋 文件清单 (共 {len(manifest)} 个文件):")
        print("=" * 70)

        for file_path, entry in sorted(manifest.items()):
            from datetime import datetime
            mtime_str = datetime.fromtimestamp(entry['mtime']).strftime('%Y-%m-%d %H:%M:%S')
            print(f"  📄 {file_path}")
            print(f"     修改时间: {mtime_str}")
            print(f"     菜谱ID: {entry.get('recipe_concept_id', 'N/A')}")
            print(f"     食材数: {len(entry.get('ingredient_ids', []))}")
            print(f"     步骤数: {len(entry.get('step_ids', []))}")
            print()

    except Exception as e:
        print(f"❌ 读取清单文件失败: {str(e)}")

def incremental_update(recipe_dir: str, output_dir: str):
    """增量更新知识图谱"""
    config = load_config()
    api_key = config["kimi"].get("api_key")
    ...
    try:
        ...
        print("🔄 开始增量更新...")
        new_count, modified_count, deleted_count, failed = builder.incremental_process_recipes(recipe_dir)
        ...
        if new_count > 0 or modified_count > 0 or deleted_count > 0:
            print("自动合并增量数据...")
            builder.export_to_neo4j_csv(output_dir, merge_batches=True)
...
使用示例:
  ...
  python batch_manager.py incremental ./HowToCook-master  # 增量更新
  python batch_manager.py manifest                  # 查看文件清单
  ...
  python batch_manager.py details -b 1              # 显示指定批次详情
    ...
    # incremental命令
    incremental_parser = subparsers.add_parser('incremental', help='增量更新知识图谱')
    incremental_parser.add_argument('recipe_dir', help='菜谱目录路径')
    
    # merge命令
    subparsers.add_parser('merge', help='合并所有批次数据')
    
    # manifest命令
    subparsers.add_parser('manifest', help='查看增量更新文件清单')
    ...
    try:
        if args.command == 'status':
            ...
            incremental_update(args.recipe_dir, args.output)
        elif args.command == 'manifest':
            show_manifest(args.output)
