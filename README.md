...
C9（**尝尝咸淡**）是一个面向烹饪领域的智能问答系统，名字取自厨房里"尝一口咸淡"的动作。它把大语言模型与烹饪知识深度结合，让用户用自然语言自由提问（"红烧肉要炖多久" / "减脂期有什么低卡素菜" / "麻婆豆腐和回锅肉共用了哪些调料"），系统能理解意图、精准检索、并生成高质量回答。

仓库对外品牌名 **What2Eat**，内部模块代号 C9。
...
### 3. 手写 ReAct Agent（不依赖 LangChain Agent）

针对 `general` / `detail` / `recommend` 三类查询实现了一个**不依赖 LangChain Agent 框架**的 ReAct 引擎：
...
基于 `CLIP` 的图文检索子模块，独立于主对话流：

```bash
# 用本仓库的扁平方便图片库建索引
python -m image_retrieval.run index-local data/C9/images

# 查询
python -m image_retrieval.run search "宫保鸡丁"
```
...
┌─────────────────────────────────────────────────────┐
│                  FastAPI 服务层                       │
│  REST API │ SSE 流式 │ 文件上传 │ 用户鉴权            │
├─────────────────────────────────────────────────────┤
│                   智能路由层                          │
│  LR → BERT → LLM 三层意图  +  多轮小 LLM 兜底        │
├──────────────────────┬──────────────────────────────┤
│  ReAct Agent 引擎    │     双引擎检索                 │
│  细/粗粒度检索工具   │  传统混合  +  图 RAG 检索      │
│                      │  (BM25+向量)  (Cypher+T2C)    │
├──────────────────────┴──────────────────────────────┤
│                  数据 & 存储层                        │
│  Neo4j（知识图谱）  Milvus（向量）  Redis（缓存/会话）│
│  PostgreSQL（长期）  SQLite（鉴权）  MinIO（Milvus 依赖）│
└─────────────────────────────────────────────────────┘
```
...
- LLM：[DeepSeek](https://platform.deepseek.com) & [月之暗面 Kimi](https://platform.moonshot.cn/)
